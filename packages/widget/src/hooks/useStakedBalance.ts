...
/** Key for the `useStakedBalance` query. `undefined` segments (no client/account
 ...
 * account and validator, so switching any of them refetches instead of
 * serving a stale value.
 */
export function useStakedBalance(validator: Address) {
  ...
  return useQuery({
    ...
    queryFn: () => {
      if (client === undefined || address === undefined) {
        throw new Error("staked-balance queryFn ran without a client or account");
      }
