...
// Same check as wagmi's `safe` connector: Safe{Wallet} runs apps in an iframe,
// and outside one the connector can never produce a provider — so it's only
// offered when the site is actually embedded.
 ...
 * (when a projectId is supplied) plus `safe` (when embedded as a Safe App),
 * EIP-6963 multi-injected discovery off so the connector list stays the
 * intended entries.
 *
